...
for (j in 1:B){
  ...
  # Out-of-bag PS estimates using splited samples
  ...
  Xs1 <- Xs;  Xs1$W <- 1
  Xs0 <- Xs;  Xs0$W <- 0
  regfit <- regression_forest(X = Xs, Y = Y, num.trees = 200, num.threads = 4, honesty=F, min.node.size=(N/65)^(4/5))
...
write.csv(out, paste0(caseid,"obs_AIPW_psC.csv"), row.names = F)
