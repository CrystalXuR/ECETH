...
for (h in 1:length(Ns)){
  ...
  for (i in 1:length(alphas)){
    ...
    for (j in 1:B){
      ...
      W <- rbinom(N, 1, 0.5)
      #Delta <- runif(N, -1, 1)
      Delta <- runif(N, 0, 2)  # mean=1, variance=1/3
...
for (h in 1:length(Ns)){
  ...
  for (i in 1:length(alphas)){
    ...
    for (j in 1:B){
      ...
      # Delta <- 2+0.5*X0     # Delta ~ N(2, 0.25) with same confounding
      # X0 <- rnorm(N, 4, 0.5)
      # Delta <- 0.5*X0      # Delta ~ N(2, 0.0625) with strong confounding 
      ...
      #integrand <- function(x) {dnorm(x, 2, 0.5)*x^2*(1-x)^2}  
      #integrand <- function(x) {dnorm(x, 2, 0.25)*x^2*(1-x)^2}  
