...
for (j in 1:B){
  ...
  psfit <- regression_forest(X = data.frame(X0, X1, X_ind), Y = W, num.trees = 200,
  ...
  # Bias 
  out[j,1] <- unlist(theta_hat - theta)
...
write.csv(out, paste0(caseid,"obs_AIPW_psC_HD.csv"), row.names = F)
